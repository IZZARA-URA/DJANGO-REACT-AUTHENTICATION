...
function MessageDetail() {
  ...
  useEffect(() => {
    let interval = setInterval(() => {
      try {
        axios.get(baseURL + '/get-messages/' + user_id + '/' + id.id)
        .then((res) => {
          setMessage(res.data)
  ...
  useEffect(() => {
    const fetchProfile = async () => {
          try {
            await axios.get(baseURL + '/profile/' + id.id)
            .then((res) => {
              setProfile(res.data)
                    ...
                    <div key={index}>
                      ...
                      {message.reciever.id === user_id &&  
                        <div className="chat-message-right pb-4" key={index}>
                          <div>
                            <img src={message.reciever_profile.image} className="rounded-circle mr-1" alt="{message.reciever_profile.full_name}" style={{objectFit:"cover"}} width={40} height={40}/>
                            <br />
                            <div className="text-muted small text-nowrap mt-2">
                                {moment.utc(message.date).local().startOf('seconds').fromNow()}
                            </div>
